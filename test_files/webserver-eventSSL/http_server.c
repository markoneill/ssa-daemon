...
void configure_context(SSL_CTX *ctx)
{
    ...
    if (SSL_CTX_use_certificate_file(ctx, CERT_FILE_A, SSL_FILETYPE_PEM) <= 0) {
        ERR_print_errors_fp(stderr);
	exit(EXIT_FAILURE);
    }

    if (SSL_CTX_use_PrivateKey_file(ctx, KEY_FILE_A, SSL_FILETYPE_PEM) <= 0 ) {
        ERR_print_errors_fp(stderr);
	exit(EXIT_FAILURE);
    }
}
	...
	while (1) {
		...
		for (n = 0; n < nfds; n++) {
			if (events[n].data.ptr == &server) {
				...
				if(SSL_set_fd(new_client->ssl, new_client->fd)<= 0){
					ERR_print_errors_fp(stderr);
				}
				if (SSL_accept(new_client->ssl) <= 0) {
            ERR_print_errors_fp(stderr);
        }
				//printf("accepted connection ssl\n");
...
int send_data(client_t* client) {
	...
	while (bytes_left > 0) {
		bytes_sent = SSL_write(client->ssl, &buffer[client->send_buf.position], bytes_left);
		//printf("%s\n", &buffer[client->send_buf.position]);
...
int send_content(client_t* client) {
	...
	while (bytes_left > 0) {
		int n = read(fd, buffer, BUFFER_MAX);
    bytes_sent = SSL_write(client->ssl, buffer, n);
		if (bytes_sent == -1) {
			if (errno == EWOULDBLOCK || errno == EAGAIN) {
				return 1; /* We've sent all we can for the moment */
			}
...
int recv_data(client_t* client) {
	...
	int bytes_read;
	while (1) {
		bytes_read = SSL_read(client->ssl, temp_buffer, BUFFER_MAX);
		if (bytes_read == -1) {
			if (errno == EWOULDBLOCK || errno == EAGAIN) {
...
int create_server_socket(char* port) {
	 ...
	 */
	ret = getaddrinfo(NULL, port, &hints, &addr_list);
	if (ret != 0) {
